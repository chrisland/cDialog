...
function cDialog(param) {
	...
	var that = this;
	if (this._opt.type == 'notif') {

		if (!this._opt.msg) {
			return false;
		}

		this._node.msg = document.createElement('div');
		this._node.msg.className = 'cDialog-msg';
		this._node.msg.innerHTML = this._opt.msg;
		this._wrap.appendChild(this._node.msg);


	} else if (this._opt.type == 'error') {
		...
		this._node.msg = document.createElement('div');
		this._node.msg.className = 'cDialog-error';
		this._node.msg.innerHTML = this._opt.msg;
		this._wrap.appendChild(this._node.msg);



	} else if (this._opt.type == 'input') {
	...
	if (this._opt.autoHide) {
		if (!isNaN(this._opt.autoHide)) {
			this._opt.autoHide = 2000;
		}
		setTimeout(function () {
			that.destroy();
		}, this._opt.autoHide)
